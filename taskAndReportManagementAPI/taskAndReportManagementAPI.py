...
class RidgeTaskAndReportAPI:
  ...
  def handle_createPeriodicTask(self, targets, task_name, periodicType, startTime, end_date, pause_start, pause_end, scenario_template=6):
    # Create a task using RidgeBot API
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks"

    # Open and modify JSON file with payload information
    if periodicType == "week": #Weekly attack mode
      with open("./JSONPayloads/createWeeklyTaskPayload.json") as createTaskJSONfile:
      ...
      with open("./JSONPayloads/createMonthlyTaskPayload.json") as createTaskJSONfile:
        createTaskPayload = json.load(createTaskJSONfile)
        createTaskPayload['name'] = task_name
        createTaskPayload['schedule']['type'] = periodicType
        createTaskPayload['schedule']['time'] = startTime
    ...
    createTaskResponse = requests.post(RidgeBotRequestURL, headers=self.RidgeBotAPIHeader, verify=False, json = createTaskPayload)
  ...
  # pause_start: pause start time, example: "05:06"
  # pause_end: pause end time, example: "11:08"
  def handle_createOneTimeTask(self, targets, task_name, periodicType, pause_start, pause_end, pending_time, scenario_template=6):
    # Create a task using RidgeBot API
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks"
    
    # Open and modify JSON file with payload information
    if periodicType == "run now": #run now mode
      with open("./JSONPayloads/createTaskPayload.json") as createTaskJSONfile:
        createTaskPayload = json.load(createTaskJSONfile)
        createTaskPayload['name'] = task_name
        createTaskPayload['schedule']['pause_start'] = pause_start
        createTaskPayload['schedule']['pause_end'] = pause_end
      ...
      with open("./JSONPayloads/createRunOnceTaskPayload.json") as createTaskJSONfile:
        createTaskPayload = json.load(createTaskJSONfile)
        createTaskPayload['name'] = task_name
        createTaskPayload['schedule']['pause_start'] = pause_start
        createTaskPayload['schedule']['pause_end'] = pause_end
  ...
  def handle_getTaskStatus(self, task_id):
    # Get the task completion status. Make sure task is finished before 
    # trying to generate a report
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks?task_id=" + task_id
    # Get Running Status
    taskStatusResponse = requests.get(RidgeBotRequestURL, headers = self.RidgeBotAPIHeader, verify = False)
    ...
    try:
      ...
      print('The task ID is invalid: ' + str(e))
      exit()
  ...
  # task_id: the id of the task
  def handle_taskStop(self, taskId):
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks/stop"

    with open("./JSONPayloads/generalTaskPayload.json") as taskStopFile:
      taskStopPayload = json.load(taskStopFile)
      taskStopPayload['task_id'] = taskId

    stopTaskResponse = requests.post(RidgeBotRequestURL, headers=self.RidgeBotAPIHeader, verify=False, json = taskStopPayload)
  ...
  # task_id: the id of the task
  def handle_taskPause(self, taskId):
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks/pause"

    with open("./JSONPayloads/generalTaskPayload.json") as taskPauseFile:
      taskPausePayload = json.load(taskPauseFile)
      taskPausePayload['task_id'] = taskId

    pauseTaskResponse = requests.post(RidgeBotRequestURL, headers=self.RidgeBotAPIHeader, verify=False, json = taskPausePayload)
  ...
  # task_id: the id of the task
  def handle_taskInfo(self, taskId):
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks/info"

    with open("./JSONPayloads/generalTaskPayload.json") as taskinfoFile:
      taskinfoPayload = json.load(taskinfoFile)
      taskinfoPayload['task_id'] = taskId

    response = requests.get(RidgeBotRequestURL, headers=self.RidgeBotAPIHeader, verify=False, json = taskinfoPayload)
  ...
  # task_id: the id of the task
  def handle_taskStatistics(self, taskId):
    
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks/statistics?task_id="+taskId

    response = requests.get(RidgeBotRequestURL, headers=self.RidgeBotAPIHeader, verify=False)
  ...
  def handle_taskDelete(self, targets):
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks/delete"

    with open("./JSONPayloads/deleteTaskPayload.json") as taskdeleteFile:
      taskdeletePayload = json.load(taskdeleteFile)
      for target in targets:
        taskdeletePayload['tasks_id'].append(target)

    deleteTaskResponse = requests.post(RidgeBotRequestURL, headers=self.RidgeBotAPIHeader, verify=False, json = taskdeletePayload)
  ...
  # task_id: the id of the task
  def handle_taskClone(self, taskId):
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks/clone"

    with open("./JSONPayloads/generalTaskPayload.json") as taskcloneFile:
      taskclonePayload = json.load(taskcloneFile)
      taskclonePayload['task_id'] = taskId

    cloneTaskResponse = requests.post(RidgeBotRequestURL, headers=self.RidgeBotAPIHeader, verify=False, json = taskclonePayload)
  ...
  # task_id: the id of the task	
  def handle_taskStart(self, taskId):
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks/start"

    with open("./JSONPayloads/generalTaskPayload.json") as taskStartFile:
      taskStartPayload = json.load(taskStartFile)
      taskStartPayload['task_id'] = taskId

    startTastResponse = requests.post(RidgeBotRequestURL, headers=self.RidgeBotAPIHeader, verify=False, json = taskStartPayload)
  ...
  # task_id: the id of the task	
  def handle_taskRestart(self, taskId):
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks/restart"

    with open("./JSONPayloads/generalTaskPayload.json") as taskRestartFile:
      taskRestartPayload = json.load(taskRestartFile)
      taskRestartPayload['task_id'] = taskId

    restartTastResponse = requests.post(RidgeBotRequestURL, headers=self.RidgeBotAPIHeader, verify=False, json = taskRestartPayload)
  ...
  def handle_saveTask(self, targets, task_name, periodicType, time, end_date, pause_start, pause_end, pending_time, scenario_template=6):
      # Create a task using RidgeBot API
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks/draft/save"

    if periodicType == "run now":
      # Open and modify JSON file with payload information
      with open("./JSONPayloads/saveTaskPayload.json") as createTaskJSONfile:
        createTaskPayload = json.load(createTaskJSONfile)
  ...
  def handle_generateReport(self, task_id, reportType, runIteration):
    # Generate and download a CSV report from the task just created
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/tasks?task_id=" + task_id
    # Get Running Status
    taskStatusHeader = {'Authorization': self.RidgeBotAuthToken, 'task_id': task_id}
    ...
    RidgeBotAPIHeader = {'Authorization': self.RidgeBotAuthToken, 'Content-Type': 'application/json'}
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/report/generate"
    with open("./JSONPayloads/createReportPayload.json") as createReportJSONfile:
    ...
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/report/"
    ...
    for report in getReportCall.json()['data']:
      report_name, reportTime = self.__convertNameToDate(report['name'])
  ...
  def handle_downloadReport(self, report_name, report_id, reportType, folderName):
    RidgeBotRequestURL = self.RidgeBotAPIURL + "/report/download"
    downloadReportCall = requests.post(RidgeBotRequestURL, \
    headers = self.RidgeBotAPIHeader, stream=True, verify = False, \
    json = {"ids": [report_id]})
  ...
  def handle_mergeTwoCSV(self, CSV_name, firstReportFolder, secondReportFolder, create_time):
    use_file = ''
